...
import { useExportJobs, type ExportItemStatus } from "../stores/export-jobs";
            ...
            return <section key={job.id} className="overflow-hidden rounded-2xl border border-border bg-surface-2">
              ...
              <div className="max-h-60 divide-y divide-border overflow-y-auto">
                {job.items.map((item) => <div key={item.clipFolder} className="flex gap-3 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1"><div className="truncate text-xs font-medium text-text" title={item.clipFolder}>{basename(item.clipFolder)}</div><div className="mt-0.5 text-[11px] text-text-muted">{t(`exportJobs.itemStatus.${item.status}`)}</div>{item.outputPath && <div className="truncate text-[11px] text-text-muted" title={item.outputPath}>{item.outputPath}</div>}{item.error && <div className="mt-1 break-words text-[11px] text-red-500">{item.error}</div>}</div>
                </div>)}
              </div>
